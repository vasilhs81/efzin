...
<h2 style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size:18px;font-weight:bold;color:rgb(0, 102, 153);display: block;" >
	<?=getTextByCode('PROPERTY_FEATURES_HEADER')?></h2>
<hr style="width: 100%;border: 1px solid #004a80;line-height: 1.8em;">
...
font-family: Verdana, Arial, Helvetica, sans-serif;line-height: 1.8em;" >
	...
	<table class="description-table"  cellpadding="4" align='center'  border=0
		...
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_CODE')?>:</th> <td><?=$row['id'] ?></td> </tr>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_CATEGORY')?>:</th><td><?=getTextByCode($row['type']) ?></td></tr>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_REGION')?>:</th> <td><?=$row['place_text'] ?></td> </tr>

		<?php if(!empty($row['price']) and $row['status_code']!='Custom'){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_PRICE')?>:</th> <td><?=$row['price_text']?></td></tr>
		<?php  } ?>

		<?php if(!empty($row['area'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_AREA')?>:</th> <td><?=$row['area_text']?></td></tr>
		<?php  } ?>


		<?php if(!empty($row['year'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_YEAR')?>:</th> <td><?=$row['year']?></td></tr>
		<?php  } ?>


		<?php if(!empty($row['rooms'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_ROOMS')?>:</th> <td><?=$row['rooms']?></td></tr>
		<?php  } ?>

		<?php if(!empty($row['wc'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_BATHS')?>:</th> <td><?=$row['wc']?></td></tr>
		<?php  } ?>

		<?php if(!empty($row['view'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_VIEW')?>:</th> <td><?=$row['view']?></td></tr>
		<?php  } ?>

		<?php if(!empty($row['level'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_LEVEL')?>:</th> <td><?=$row['level']?></td></tr>
		<?php  } ?>
		<?php if(!empty($row['total_levels'])){?>
		<tr><th><?=getTextByCode('PROPERTY_FEATURES_TOTAL_LEVELS')?>:</th> <td><?=$row['total_levels']?></td></tr>
		<?php  } ?>

		<?php if(!empty($row['status_code'])){?>
		<tr><th><?php
				if($row['status_code']!='Custom') echo getTextByCode('FEATURE_AVAIL');
				else echo getTextByCode('PROPERTY_FEATURES_AVAIL_PRICES')?>:</th> <td>
				<?php
				if($row['status_code']=='Free' || $row['status_code']=='Occupied') {
					echo getTextByCode($row['status_code']);
				}elseif($row['status_code']=='Custom'){
					echo ($row['custom_prices']);
				}
				else{
					...
					if(count($avail)>0){
						...
						echo "return false;";
							echo '><img src="images/cal.png" style="vertical-align: middle" > '. getTextByCode('PROPERTY_FEATURES_AVAIL').'</div>';

					 }else echo '-'; }?></td></tr>
		<?php  } ?>

		<?php if(!empty($row['amenities'])){?>
			<tr><th><?=getTextByCode('PROPERTY_FEATURES_AMENITIES')?>:</th> <td><?php
					$amenities=explode(',',$row['amenities']);
					foreach($amenities as $amenity){
						echo "<div style='display:inline-block;padding:0px 8px;margin-right:4px;width:auto;margin-bottom:2px;
max-width:160px;height:30px;overflow: hidden;white-space: nowrap;line-height: 2.5em;vertical-align: bottom;
		...
		<?php if(!empty($row['af'])){?>
			<tr><th><?=getTextByCode('PROPERTY_FEATURES_ADDITIONAL')?>:</th> <td><?=$row['af']?></td></tr>
		<?php  } ?>
...
width:880px;padding-left:20px;display: block;
	...
	<h2 style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size:18px;font-weight:bold;color:rgb(0, 102, 153);height:22px;line-height: 1.8em;" >
		<?=getTextByCode('PROPERTY_FEATURES_DESCRIPTION')?></h2>
	<hr style="width: 100%;border: 1px solid #004a80;">

	<div class="item-description-text" style="line-height: 1.6em;">
		<?=$row['description'] ?></div>
</div>
...
<div style="width: 100%;height: auto;line-height: normal;">
	<button type="submit"
	        ...
	        class="large-book-button"><?=getTextByCode('PROPERTY_FEATURES_BOOK')?></button>
<div class="clear8">&nbsp;</div>
	...
	<div style="width: 100%;height: auto;line-height: normal;">
		...
		<div style="width: 100%;height: auto;line-height: normal;">
			<h2 style="font-family: Verdana, Arial, Helvetica, sans-serif;
			font-size:18px;font-weight:bold;color:rgb(0, 102, 153);height:22px;display: block;width: 100%;" >
				<?=getTextByCode('PROPERTY_FEATURES_PHOTOS')?></h2>
		...
		<a href="index.php"  class="link2" style="margin-left:12px;" > « <?=getTextByCode('PROPERTY_FEATURES_HOME')?> </a>
  ...
  margin-right: auto ;
		 " >
               ...
               ">
					...
					<h1><?=getTextByCode('PROPERTY_FEATURES_AVAIL')?> (<?=$row['id']?>)</h1>
