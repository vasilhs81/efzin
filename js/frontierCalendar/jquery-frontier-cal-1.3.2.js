			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayIndex < firstDayWkIndex){
					...
					if(showTodayStyle && dayNum == currentDayNum){
						...
						calWeekHeaderCellObj.setHtml("<span style='color:#F90;font-weight:bold;'> "+dayNum+"</span>");
						calWeekHeaderCellObj.addClass("JFrontierCal-WeekHeaderCell-Today");
					}					
					dayNum += 1;
				}
				if(dayIndex == 6){
...
calWeekHeaderCellObj.setHtml("<span style='color:#F90;font-weight:bold;'> "+dayNum+"</span>");
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(showTodayStyle && dayNum == currentDayNum){
					...
					//calWeekHeaderCellObj.setHtml(/*"Today - "+*/dayNum+"&nbsp;");
//					calWeekHeaderCellObj.setHtml("Today - "+dayNum+"&nbsp;");
calWeekHeaderCellObj.setHtml("<span style='color:#F90;font-weight:bold;'>(Today) "+dayNum+"</span>");
						calWeekHeaderCellObj.addClass("JFrontierCal-WeekHeaderCell-Today");
		...
		this.renderAgendaDivElement = function(agi,displayMessage,startDayObject,endDayObject,leftEnd,rightEnd){
			...
			if(nextY > 0){
				...
				/*
				var dd = $("<img/>");
				dd.attr("src","events/"+agi.getAgendaId()+".jpg");
				dd.attr("width",startDayObject.getWidth());
				dd.attr("height",startDayObject.getHeight());
				startDayObject.appendHtml(dd);
				*/
				var dd = $("<a/>");
				dd.attr("href","javascript:top.location='event.php';");
				dd.attr("title",displayMessage);
				dd.html("<img src='events/"+agi.getAgendaId()+".jpg' width='"+(startDayObject.getWidth())+"' height='"+(startDayObject.getHeight())+"'>");
				//dd.html("skata");
				startDayObject.appendHtml(dd);
				
				
				//startDayObject.setCss("background-image","url(a1.jpg)");
				//startDayObject.setCss("background-position","");
				//alert(startDayObject.getCss("background-image"));
		...
		this.buildCalendarDayCell = function(){
			...
			//dayCell.css("background-image","url(a1.jpg)");
