...
<form action="<?=$_SERVER['PHP_SELF']?>" method="post" id="inquiry-form" >
	<div id="track-details"   style="float:right;margin-left: 12px;width: 100%;
		...
		<div class="user-label-box" style="border-bottom: 2px solid #004a80;font-weight: bold;color:#004a80;width:96%;">
			<?=getTextByCode('CUSTOMER_INFO_HEADER')?></div>
...
<!-- Kritiria Zhthshs ---------------->
		...
		<div class="clear8">&nbsp;</div>
		<div class="user-label-box" style="border-bottom: 2px solid #004a80;font-weight: bold;color:#004a80;width:96%;">
			<?=getTextByCode('ENQUIRY_CRITERIA_HEADER')?></div>
		...
		<div style="width: 50%;float:left;text-align: left;display: block;">
			<legend class="user-label-box"><?=getTextByCode('CRITERIA_PRICE')?>:&nbsp;</legend>
				<input type="text"  style="width:12%;clear: none;display: inline-block;"  class="user-input-box" name="price_from" placeholder="<?=getTextByCode('CRITERIA_PRICE_FROM')?>"
						<?php if(!empty($_POST['price_from'])) echo "value='$_POST[price_from]'" ?>
						  >&nbsp;  <input  class="user-input-box" type="text" style="width:12%;clear: none;display: inline-block;" name="price_to" placeholder="<?=getTextByCode('CRITERIA_PRICE_TO')?>"
...
&nbsp;<select class="user-input-box" style="height:53px;width: 60%;clear: none;display: inline-block;" name="property_charge_period" id="property_charge_period" >
		...
		<div style="width: 50%;text-align: left;display: inline-block;">
			<legend class="user-label-box"><?=getTextByCode('CRITERIA_AREA')?> :&nbsp;</legend>
					<input style="width:12%;clear: none;display: inline-block;"   class="user-input-box"
						<?php if(!empty($_POST['area_from'])) echo "value='$_POST[area_from]'" ?> placeholder="<?=getTextByCode('CRITERIA_AREA_FROM')?>"
			...
			placeholder="<?=getTextByCode('CRITERIA_AREA_TO')?>" type="text" name="area_to">
			</div>
...
<!--------------------------------- SXOLIA--------------------------->
		...
		<label for="userEmailNotify"><?=getTextByCode('CRITERIA_WISH')?></label>
